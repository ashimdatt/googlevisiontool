...
def get_event_images(applicationId):
    ...
    conn = psycopg2.connect("dbname='analytics' user='name' host='server' password='pass' port=''")
    cur = conn.cursor()
    ...
    image_list_sql = """SELECT 'url || LOWER(ucii.externalimageid) || '.jpg' AS ImageURL, LOWER(ucii.externalimageid) || '.jpg' AS FileName FROM PUBLIC.Ratings_UserCheckIns UCI JOIN PUBLIC.Ratings_UserCheckInImages UCII ON UCI.CheckInId = UCII.CheckInId WHERE UCI.ApplicationId = UPPER('""" + applicationId + """');"""
